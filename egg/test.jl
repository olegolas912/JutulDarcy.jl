...
function plot_well_results(response, well_names, ylabel = "$response")
    fig = Figure(size = (1000, 400))
    ys = si_unit(:day)
    ax = Axis(fig[1, 1], xlabel = "Time / days", ylabel = ylabel)
    i = 1
    for well_name in well_names
        ...
        lines!(ax, time_jutul ./ day, abs.(qoi),
               color = i,
               colorrange = crange,
...
water_plot = plot_well_results(:wrat, prod, "Surface water rate / m³/day")
save("water_rate.png", water_plot)
...
writedlm("time_steps.csv", result.time)
open("reservoir_states.txt", "w") do io
    for (i, state) in enumerate(result.states)
        write(io, "Step $i, Pressure: $(state[:Pressure]), Saturation: $(state[:Saturations])\n")
    end
end
all_states = result.states

println("Number of states: ", length(all_states))
